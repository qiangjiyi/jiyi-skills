...
```bash
bash scripts/run.sh agent_delete.py --codex-ghosts   # Codex 侧栏幽灵清扫（App 已退出，门禁可过）
```
...
——某个 Claude CLI 异常退出没清掉自己的状态文件时留下的残渣。同时清扫 Codex 侧栏状态文件
里的**全部幽灵条目**（UI 有 id、threads 表已无的残留，实测多由 Codex 原生删除或旧版工具
遗留）——哪怕本次一个会话都不删，跑完流程侧栏也是干净的。这些都是删会话/退出时没收
...
Step 3 ② 固定跑 `agent_delete.py --codex-ghosts` 全量清扫；③ 报告上的 👻 一键清理按钮
作为兜底入口。**幽灵清理不放在扫描阶段**：扫描是只读铁律（报告闸门与竞态校验都依赖快照
不可变），且扫描时 ChatGPT 通常仍在运行（写门禁必然拒绝）——检测在扫描、清理在关 App
之后，各归其位。
